...
<AccordionGroup>
  ...
  <Accordion title="`geniex` exits with 'device is missing CPU features geniex requires'">
    ...
    If those features are absent, install the **baseline** build instead. It is compiled for plain `armv8.0-a` with `-moutline-atomics`, ships CPU inference only (no QAIRT/NPU, no OpenCL/GPU, no Hexagon), and is correspondingly slower — ggml falls back to its fp32 quant kernels without `dotprod`/`fp16`. `install.sh` picks it automatically when `atomics` is missing from `/proc/cpuinfo`; force it with `--baseline`:

    ```bash bash
    curl -fsSL https://raw.githubusercontent.com/qualcomm/GenieX/main/cli/release/linux/install.sh | sh -s -- --baseline
    ```

    If the baseline build still fails, report it in [GitHub Issues](https://github.com/qualcomm/GenieX/issues) or Slack with the output above.
  </Accordion>
</AccordionGroup>
